...
export async function getUserDetails(req, res) {
    console.log('get user details called, token passed: ',req.token);
    let response = await fetch('https://platform.flipkart.net/1/authorization/auth?grantToken='+req.token+'&clientId=playground.pizza&clientSecret=4FKzLsQDixo2tcP3nyzp8yq6jQq57cgy8ECQkiMHE7SlMiHy',
    //let response = await fetch('https://platform.flipkart.net//1/dummy',
    {method: 'GET',
        headers: {
            'secureToken': getSecureToken()
        }
    }).then( function (response) {
        console.log('identity token response json',response);
    }).catch((error) => {
...
let getSecureToken = function () {
    ...
    console.log('key generated',sJWT);
    return sJWT;
};